...
const Event = ({ data, compact }: { compact?: boolean; data: LogEvent }) => {
  const transform = (node: Element) => {
    if (node.type === "tag" && node.name === "strong") {
      return (
        <HighlightedText key={get(node.children[0], "data")}>
          {get(node.children[0], "data")}
        </HighlightedText>
      );
    }
  ...
  const options = { replace: transform } as HTMLReactParserOptions;
      ...
      return compact ? (
        <>
          ...
          <div className="mt-4">
            ...
            {data.tags?.map((label: string, index: number) => (
              <Tag
                key={label + index}
                className="mx-1"
                prefix
