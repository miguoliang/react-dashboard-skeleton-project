...
const Billing = () => {
  const [data, setData] = useState<typeof settingBillingData>({});
  ...
  const onEditCreditCard = (card: PaymentMethod, type: string) => {
  ...
  const onCardUpdate = (
    ...
    if (cardValue.primary) {
      paymentMethodsValue.forEach((card: PaymentMethod) => {
        card.primary = false;
      });
    }
    ...
    paymentMethodsValue = paymentMethodsValue.map((card: PaymentMethod) => {
    ...
    let cardTemp = {};
    paymentMethodsValue = paymentMethodsValue.filter((card: PaymentMethod) => {
      if (card.primary) {
        cardTemp = card;
      }
      return !card.primary;
