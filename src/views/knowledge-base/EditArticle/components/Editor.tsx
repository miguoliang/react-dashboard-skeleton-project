...
const Editor = ({ mode }: { mode: string }) => {
  ...
  const onComplete = async (
    value: { title: string; content: string; category: string },
    setSubmitting: (v: boolean) => void,
  ) => {
    setSubmitting(true);
    const newData = { ...article, ...value, categoryLabel };
    await apiPostArticle(newData);
    setSubmitting(false);
    toast.push(
  ...
  useEffect(() => {
    if (
      !categoryList.some((cat) => cat.value === categoryValue) &&
      categoryLabel &&
      categoryValue
    ) {
  ...
  return (
    <Formik
      ...
      onSubmit={(values, { setSubmitting }) => {
        onComplete(values, setSubmitting).then(noop);
      }}
    >
