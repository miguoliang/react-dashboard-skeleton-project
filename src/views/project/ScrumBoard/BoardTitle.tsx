...
const RenameForm = ({
  ...
  const onFormSubmit = (newTitle: string) => {
    if (ordered.some((elm: string) => elm === newTitle)) {
      closeRenameForm();
      return;
    }
    ...
    const newOrder = ordered.map((elm: string) => {
      if (elm === title) {
        return newTitle;
      }
      return elm;
...
const BoardTitle = (props: {
  ...
  const onDelete = () => {
    const newOrder = ordered.filter((elm: string) => elm !== title);
    const newColumns: Record<string, Card[]> = {};
      ...
      {renameActive ? (
        ...
        <>
          ...
          <Dropdown placement="bottom-end" renderTitle={<EllipsisButton />}>
            <DropdownItem eventKey="renameBoard" onClick={onRenameActive}>
              <span className="text-lg">
                <HiOutlinePencil />
              </span>
              <span className="ml-2 rtl:mr-2">Rename</span>
            </DropdownItem>
            <DropdownItem eventKey="addTicket" onClick={onAddNewTicket}>
              <span className="text-lg">
                <HiOutlinePlusCircle />
              </span>
              <span className="ml-2 rtl:mr-2">Add Ticket</span>
            </DropdownItem>
            <DropdownItem eventKey="deleteBoard" onClick={onBoardDelete}>
              <span className="text-lg">
                <HiOutlineTrash />
              </span>
      ...
      >
        <div className="px-6 pb-6 pt-2 flex">
          ...
          <div className="ml-4 rtl:mr-4">
            ...
            <p>
              Are you sure you want to delete this board? All the tickets under
              this board will be deleted as well. This action cannot be undone.
            </p>
          </div>
        </div>
